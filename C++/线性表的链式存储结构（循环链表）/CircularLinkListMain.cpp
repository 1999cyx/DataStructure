...
void Realization(int sel) {
    ...
    switch (sel) {
        ...
        case 2:
            cout << "\t尾插发插入数据" << endl;
            input("待插入的数据：", data);
            myList.append_last(data);
            break;
        case 3:
            cout << "\t插入到第pos个位置" << endl;
            input("位置pos：", x);
            input("待插入的数据：", data);
            if (!myList.append_pos(x, data))
                cout << "插入失败" << endl;
            break;
        case 4:
            cout << "\t删除表头数据" << endl;
        ...
        case 12:
            ...
            input("待输出数据位置：", pos);
            if (myList.get_pos_data(pos, data))
